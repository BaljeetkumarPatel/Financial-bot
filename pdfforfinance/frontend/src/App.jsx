...
function Message({ msg }) {
  ...
  return (
    <div className={`group flex gap-4 px-4 py-5 ${isUser ? "bg-transparent" : "bg-gray-50"}`}>
      {/* Avatar */}
      <div
        className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-sm font-bold mt-0.5 ${
          isUser ? "bg-[#1FA2B6] text-white" : "bg-[#072146] text-white"
        }`}
      >
        {isUser ? "U" : "AI"}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-gray-500 mb-1">{isUser ? "You" : "Finance Assistant"}</p>
...
function TypingIndicator() {
  return (
    <div className="flex gap-4 px-4 py-5 bg-gray-50">
      <div className="w-8 h-8 rounded-full bg-[#072146] text-white flex items-center justify-center text-sm font-bold shrink-0">
        AI
      </div>
      <div className="flex items-center gap-1 mt-2">
        {[0, 1, 2].map((i) => (
          <span
            key={i}
            className="w-2 h-2 bg-[#1FA2B6] rounded-full animate-bounce"
...
export default function App() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      updateSession(sid, (s) => ({ ...s, messages: [...s.messages, aiMsg] }));
    } catch {
      updateSession(sid, (s) => ({
        ...s,
        messages: [
          ...s.messages,
          { role: "ai", text: "⚠️ Backend not reachable. Make sure the server is running on port 8001.", time },
        ],
      }));
    } finally {
      setLoading(false);
  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden font-sans">
      ...
      <aside
        ...
        } shrink-0 bg-[#0f1117] text-white flex flex-col transition-all duration-200 overflow-hidden`}
      >
        {/* New chat button */}
        <div className="p-3 border-b border-white/10">
          <button
            onClick={createSession}
            className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-white/10 text-sm font-medium transition"
          >
          ...
          {sessions.map((s) => (
            <div
              ...
              className={`group flex items-center justify-between px-3 py-2.5 rounded-lg cursor-pointer text-sm transition ${
                s.id === activeId ? "bg-white/15 text-white" : "text-gray-300 hover:bg-white/10"
              }`}
            >
              <span className="truncate flex-1">💬 {s.title}</span>
              <button
                onClick={(e) => deleteSession(s.id, e)}
                className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-400 ml-2 text-xs transition"
              >
                ✕
              </button>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="flex items-center gap-3 px-4 py-3 border-b bg-white shrink-0">
          <button
            ...
            className="text-gray-500 hover:text-gray-800 transition p-1 rounded"
        ...
        <div className="border-t bg-white px-4 py-4 shrink-0">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-xs text-gray-400 text-center mt-2">
              Answers are grounded in your uploaded financial documents only.
            </p>
          </div>
        </div>
      </div>
